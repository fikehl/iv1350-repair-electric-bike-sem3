...
/**
 * Unit tests for {@link RepairOrder}. The repair order entity is read by
 * the tests through its {@link RepairOrder#toDTO()} snapshot, so the test
 * does not have to break encapsulation by adding extra getters to the SUT.
 */
public class RepairOrderTest {
    ...
    public void testToDTOOfNewOrderHasNoDiagnosticDescription() {
        ...
        assertNull(snapshot.getDiagnosticDescription(),
    ...
    public void testToDTOOfNewOrderHasNoProposedTasks() {
        RepairOrderDTO snapshot = repairOrder.toDTO();
        assertTrue(snapshot.getProposedTasks().isEmpty(),
    ...
    public void testInitialTotalCostIsZero() {
        RepairOrderDTO snapshot = repairOrder.toDTO();
        assertEquals(new Amount(0), snapshot.getTotalCost(),
    ...
    public void testCustomerIsStored() {
        RepairOrderDTO snapshot = repairOrder.toDTO();
        assertSame(customer, snapshot.getCustomer(),
    ...
    public void testProblemDescriptionIsStored() {
        RepairOrderDTO snapshot = repairOrder.toDTO();
        assertEquals("Brakes squeaking.", snapshot.getProblemDescription(),
    ...
    public void testCreationTimeIsCloseToNow() {
        ...
        long secondsBetween = ChronoUnit.SECONDS.between(
                snapshot.getCreationTime(), LocalDateTime.now());
    ...
    public void testToDTOAfterAddingReportContainsDiagnosticDescription() {
        ...
        RepairOrderDTO snapshot = repairOrder.toDTO();
        assertEquals(report.getDescription(), snapshot.getDiagnosticDescription(),
    ...
    public void testTotalCostMatchesDiagnosticReportAfterAdding() {
        ...
        repairOrder.addDiagnosticReport(report);
        RepairOrderDTO snapshot = repairOrder.toDTO();
        assertEquals(report.getTotalCost(), snapshot.getTotalCost(),
